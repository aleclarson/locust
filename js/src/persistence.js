var SortedArray, assert, async, inArray, log, lotus, ref, sync;
...
module.exports = {
  ...
  fromJSON: function() {
    ...
    return async.read("lotus-cache.json").then(function(json) {
      ...
      ignoredErrors = ["Module with that name already exists!"];
        ...
        return async.each(loadedModules.array, function(arg) {
          ...
          return module.dependers = sync.reduce(dependers, {}, function(dependers, name) {
            ...
            if (dependerModule != null) {
              ...
              log.moat(1).white("Module error: ").red(name).moat(0).gray("Could not find module named '" + name + "'!").moat(1);
            }
            return dependers;
          });
        });
